...
if(!get_index_table($db, 0)) {
    $make_index = 'CREATE TABLE `tables_index` (rid MEDIUMINT AUTO_INCREMENT NOT NULL PRIMARY KEY, name VARCHAR(30)) TYPE=MyISAM;';
    $db->sql_query($make_index) or die('Could not create index table tables_index <br />' . $makeIndex . '<br />' . $db->sql_error());;
}
...
while(!feof($team_page_handle))
{
    ...
    if($current_line == '' || $current_line == ' ' || $lines_count <= 2 || $lines_count == 4 || ($lines_count >= 7 && $lines_count <= 9)) { // if the line sucks
	...
	$db->sql_query($tables_update) or die('Could not insert row for ' . $current_table_name . ' into tables_index<br />' . $tables_update . '<br />' . mysql_error());
        $tables_array = get_index_table($db, 1);
